...
<div>
    <section class="section">
        ...
        @if($sub_menu_name)
        <div class="section-header mb-1 rounded-top" style="border-radius: 0">
            ...
            <div class="section-header-breadcrumb">
                ...
                <div class="breadcrumb-item"><a href="{{ route('pelaksanaan.'.Str::lower($division).'.index') }}">Divisi {{$division}}</a></div>
                <div class="breadcrumb-item"><a href="{{ route('pelaksanaan.'.Str::lower($division).'.'.Str::snake($page_attribute['title']).'.index') }}">{{$page_attribute['title']}}</a></div>
            </div>
        </div>
        
        <div class="card rounded-bottom" style="border-radius: 0">
            <div class="card-body py-2">
                <h4 class="text-dark mb-0" style="font-size: 16px;"><i class="fas fa-folder custom-fa-1x-2 custom-bg-folder mr-2"></i> {{$sub_menu_name}}</h4>
            </div>
        </div>
        @else
        ...
        <div x-data="{ control_tabs: @entangle('control_tabs') }" class="section-body">
            ...
            <div :class="{ 'd-none': !control_tabs.detail }" class="row d-none">
                ...
                <div class="col-12 mb-4">
                    @if ($menu_type_view == 'week' && $control_tabs['week_list'])
                    <button id="btn-back-week" wire:click="clearGroupWeek" wire:target="clearGroupWeek" wire:loading.class="disabled btn-progress" class="btn btn-warning"><i class="fas fa-arrow-left"></i> Back</button>
                    @else
                    <button id="btn-back-group" x-on:click="control_tabs.list = true;control_tabs.detail = false;" class="btn btn-warning"><i class="fas fa-arrow-left"></i> Back</button>
                    @endif
                </div>
                <div class="col-12">
                    <div class="row">
                        @forelse($selected_item_group as $key_group => $item_group)
                        @if ($menu_type_view == 'week' && !$control_tabs['week_list'])
                        <div class="col-12 col-sm-6 col-md-6 col-lg-3">
                            <div id="overlay-card-week-{{$key_group}}" wire:target="setGroupWeek({{$key_group}})" wire:loading.flex class="custom-card-overlay" style="display: none">
                                <i class="fas fa-3x fa-sync-alt fa-spin fa-3x"></i>
                            </div>
                            <div wire:click="setGroupWeek({{$key_group}})" class="card custom-card-folder folder-event">
                                <div class="card-body">
                                    <div class="text-center">
                                        <i class="fas fa-folder custom-fa-10x custom-bg-folder"></i>
                                    </div>
                                    <div class="w-100 mt-2">
                                        <h6 class="text-uppercase mb-0">Minggu ke-{{$key_group}}</h6>
                                    </div>
                                </div>
                            </div>
                        </div>
                        @elseif($menu_type_view == 'single' || ($menu_type_view == 'week' && $control_tabs['week_list']))
                ...
                <div :class="{ 'd-none': !control_tabs.sector_detail }" class="col-12 d-none">
                    <div class="row">
                        <div class="col-12 mb-4">
                            @if ($menu_type_view == 'week' && $control_tabs['sector_week_list'])
                            <button id="btn-back-week" wire:click="clearGroupWeek(true)" wire:target="clearGroupWeek(true)" wire:loading.class="disabled btn-progress" class="btn btn-warning"><i class="fas fa-arrow-left"></i> Back</button>
                            @else
                            <button x-on:click="control_tabs.sector_list = true;control_tabs.sector_detail = false;$wire.clearSector()" class="btn btn-warning"><i class="fas fa-arrow-left"></i> Back</button>
                            @endif
                        </div>
                        @forelse ($selected_item_sector_group as $key_sector => $sector_item)
                        @if ($menu_type_view == 'week' && !$control_tabs['sector_week_list'])
                        <div class="col-12 col-sm-6 col-md-6 col-lg-3">
                            <div id="overlay-card-week-sector-{{$key_sector}}" wire:target="setSectorGroupWeek({{$key_sector}})" wire:loading.flex class="custom-card-overlay" style="display: none">
                                <i class="fas fa-3x fa-sync-alt fa-spin fa-3x"></i>
                            </div>
                            <div wire:click="setSectorGroupWeek({{$key_sector}})" class="card custom-card-folder folder-event">
                                <div class="card-body">
                                    <div class="text-center">
                                        <i class="fas fa-folder custom-fa-10x custom-bg-folder"></i>
                                    </div>
                                    <div class="w-100 mt-2">
                                        <h6 class="text-uppercase mb-0">Minggu ke-{{$key_sector}}</h6>
                                    </div>
                                </div>
                            </div>
                        </div>
                        @elseif ($menu_type_view == 'single' || ($menu_type_view == 'week' && $control_tabs['sector_week_list']))
                        <div class="col-12 col-sm-6 col-md-4 col-lg-3">
                            <div class="card shadow-sm custom-card-folder">
                                <article class="article article-style-b mb-0">
                                    <div class="article-header">
                                        ...
                                        <a class="main-popup-link" href="{{ route('files.image.stream', ['path'=>$sector_item['base_path'], 'name' => $sector_item['image_name']]) }}">
                                            <div class="article-badge custom-article-badge w-100">
                ...
                <form wire:submit.prevent="addItem" x-data="{ isUploading: false, progress: 0 }" x-on:livewire-upload-start="isUploading = true" x-on:livewire-upload-finish="isUploading = false" x-on:livewire-upload-error="isUploading = false" x-on:livewire-upload-progress="progress = $event.detail.progress">
                    <div class="modal-body">
                        ...
                        <div class="form-group mb-3">
                            <label>Tipe</label>
                            <select wire:model="selected_list" class="form-control" required>
                                @foreach ($option_lists as $option_list)
                                <option value="{{$option_list['slug_name']}}">{{$option_list['menu_name']}}</option>
                                @endforeach
                            </select>
                        </div>
                        @error('selected_list')
